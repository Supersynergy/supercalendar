...
### Added
- **`cloudflare/` — native Cloudflare Pages app** (Astro 5 + React 19 island + Turso/libSQL). Deploys to `*.pages.dev` via dashboard **git-connect, no CLI login** (set root directory `cloudflare`, build `bun run build`, output `cloudflare/dist`). 0KB-JS-first: only the calendar island hydrates (2.83 kB gzip). Month view + create/delete events, optimistic with rollback, 8s edge-safe polling. Same event shape as the Next app; point it at the **same Turso DB** to share events. Verified: build green, full CRUD, `astro check` 0/0/0. See `cloudflare/README.md`.
- **Cloudflare deploy (Workers, via OpenNext)** — `@opennextjs/cloudflare` adapter, `wrangler.jsonc`, `open-next.config.ts`, and `bun run cf:build|cf:preview|cf:deploy` scripts. Build verified offline (`.open-next/worker.js`, 1.43 MB gzip; `wrangler deploy --dry-run` clean). Persistence = Turso/libSQL via `DATABASE_URL` + `DATABASE_AUTH_TOKEN` secret. Guide: `deploy/cloudflare.md`.
- **Edge-safe sync backstop** — `calendar-context.tsx` now polls `/api/events` every 8s (paused while the tab is hidden) alongside the SSE layer, so cross-device sync works on Cloudflare Workers where the in-process SSE bus can't reach across isolated invocations. SSE stays the instant path on single-instance Node/Docker.
- **Self-hosted Inter font** — `src/styles/fonts.ts` switched from `next/font/google` to `next/font/local` (bundled variable woff2). Reproducible offline/edge builds, no runtime Google Fonts request (DSGVO-friendly).
